...
其中 `watchvuln-web` 服务用于启动 Web UI（端口映射为 `8080:8080`）。
...
Docker 下启动 Web UI 需要把命令切换为 `web`，并暴露端口（建议同时挂载配置文件，方便在 Web 端修改并保存到文件）：
...
docker run --restart always -d \
  ...
  -e WEB_UI_DIR=/app/webui \
  -e DB_CONN=sqlite3:///app/data/vuln_v3.sqlite3 \
  -e ADMIN_USERNAME=admin \
  -e ADMIN_PASSWORD=change-me \
  -e SESSION_SECRET=change-me-to-a-long-random-secret \
  zemal/watchvuln:latest -c /app/config.yaml web
```

访问 `http://127.0.0.1:8080/`，登录后进入“配置”页即可用表单修改并保存配置（敏感字段会以 `********` 占位）。
...
```bash
docker run --restart always -d \
  -e SLACK_WEBHOOK_URL=https://example.com/your-slack-webhook \
...
GLOBAL OPTIONS:
   ...
   --slack-webhook-url value, --sw value      specify slack webhook url
...
```
$ ./watchvuln --sw https://example.com/your-slack-webhook --sc '#your_channel'  -i 30m
```
...
项目提供 `web` 子命令，用于启动同域的 Web UI + API（用于查看漏洞列表与详情）：
...
配置文件中需要包含 Web 相关字段（示例见 [config.example.yaml](./config.example.yaml)）：
...
说明：
- 发布的 Go 二进制已内置 Web UI 静态资源，不需要额外拷贝 `webui/` 目录也能打开页面
- 如需自定义前端资源目录（例如自行替换前端文件），再配置 `web_ui_dir` 指向你的目录即可（会优先读取磁盘目录）

启动后访问 `http://127.0.0.1:8080/`，登录后进入“配置”页即可用表单方式修改配置：
- 每个配置项都会提供说明提示
- 敏感字段会以 `********` 占位显示；保存时留空或保持 `********` 会保留原值，避免误覆盖
- 保存后会尝试运行时应用部分配置（如重连 DB / 更新鉴权 / 切换 UI 目录）；`web_listen` 需要重启进程生效
